          ...
          .within(() => {
            ...
            cy.get("@orderid")
              .then((orderid) => {
                cy.intercept(
                  "POST",
                  "https://v2.nushop-dashboard.kaip.in/api/order-process/report/invoices-v2/created"
                ).as("res");
                cy.get(
                  ".PrintPackViewCard_card__Y4Iaa > :nth-child(1) > .rs-col-lg-3 > .action-btns-wrapper > :nth-child(2)"
                ).click();
              ...
              .then(() => {
                cy.get("@orderid").then((orderid) => {
                  cy.get("@invoice").then((invoice) => {
                    ...
                    cy.wait(5000);
                    cy.task("readPdf", InvoiceContent).should(
                      "contain",
                      orderid
                    );
                  });
