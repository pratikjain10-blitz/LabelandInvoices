...
describe("AdminLimitStore", function () {
  var new_Clubbed_dimensions;
  var ManualLP_charges = "";
  it("Calculating Dimensions+ForwardRatesCalculation", function () {
    ...
    if (Sku_Weight_01 > Volumetric_Weight01) {
      ...
      cy.log("Weight of first Product", Final_Weight_01);
    } else {
    ...
    if (Sku_Weight_02 > Volumetric_Weight02) {
      ...
      cy.log("Weight of first Product", Final_Weight_02);
    } else {
    ...
    cy.get("@n").then((n) => {
      ...
      cy.get("@baseCharge").then((baseCharge) => {
        ...
        cy.get(
          ...
          var percentage_Surcharge_number = $n1.text().substring(0, 2);
          cy.log(percentage_Surcharge_number);
          percentage_Surcharge = (percentage_Surcharge_number / 100) * SP;
          cy.log(percentage_Surcharge);
          cy.wrap(percentage_Surcharge).as("percentage_Surcharge");
        });
        cy.get("@Payment_COD_charge").then((Payment_COD_charge) => {
          cy.get("@percentage_Surcharge").then((percentage_Surcharge) => {
            cy.log("percentage_Surcharge", percentage_Surcharge);
            cy.log("Payment_COD_charge", Payment_COD_charge);
            if (percentage_Surcharge > Payment_COD_charge) {
              Payment_COD_charge = percentage_Surcharge;
            ...
            cy.then(() => {
              cy.get("@baseCharge").then((baseCharge) => {
                cy.get("@Payment_COD_charge").then((Payment_COD_charge) => {
                  var Card_Calculation_PaymentBased =
                    parseFloat(Payment_COD_charge);
