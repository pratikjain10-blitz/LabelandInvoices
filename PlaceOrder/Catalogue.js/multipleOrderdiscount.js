...
cy.wait(2000)
//discount tests start here
cy.get('p[class="Text_body2__0FftJ Text_primary-colored__qnpSF Text_ml-sm__WjbvN Text_ellipsis__vYMZP"]').each(($e1,index,$list)=>
{
var c = $e1.text()
if(c==='WD Control SingleO 1')
{
cy.get('.rs-picker-toggle-placeholder > .Text_body2__0FftJ').click().then(($p1)=>
{
    cy.get('[aria-selected="false"] > .rs-picker-select-menu-item').each(($z1,number,list)=>
    {
if($z1.text()==='Discount amount per quantity')
{
    cy.get('[aria-selected="false"] > .rs-picker-select-menu-item').eq(number).click()
}
    })
})
cy.get('.EditableProductCard_discount-input-style__wx3FL').eq(index).type('100')
cy.get('.rs-flex-box-grid-item-4 > .Button_button-ghost__rieSu').eq(index).click()
}
 if(c==='WD Test2 ClubbedO 1')
{
cy.get('.rs-picker-toggle-placeholder > .Text_body2__0FftJ').eq(index).click().then(($p1)=>
{
    cy.get('[aria-selected="false"] > .rs-picker-select-menu-item').each(($z1,number,list)=>
    {
if($z1.text()==='Percentage discount per quantity')
{
    cy.get('[aria-selected="false"] > .rs-picker-select-menu-item').eq(number).click()
}
    })
cy.get('.EditableProductCard_discount-input-style__wx3FL').eq(index).type('10')
cy.get('.rs-flex-box-grid-item-4 > .Button_button-ghost__rieSu').eq(index).click()
})
}
})
