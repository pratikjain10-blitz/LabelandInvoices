...
describe("Placeorder/Catalogue/SingleNormalOrder/Discountfunctionality", function () {
  ...
  it("DiscountAmount", function () {
    ...
    cy.get(
      'p[class="Text_body2__0FftJ Text_primary-colored__qnpSF Text_ml-sm__WjbvN Text_ellipsis__vYMZP"]'
    ).each(($e1, index, $list) => {
      var c = $e1.text();
      if (c === "WD Control SingleO 1") {
        cy.get(".rs-picker-toggle-placeholder > .Text_body2__0FftJ")
          ...
          .then(($p1) => {
            cy.get(
              '[aria-selected="false"] > .rs-picker-select-menu-item'
            ).each(($z1, number, list) => {
        ...
        cy.get(".EditableProductCard_discount-input-style__wx3FL")
          .eq(index)
          .type("100");
