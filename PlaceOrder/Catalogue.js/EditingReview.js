...
describe('Placeorder/Catalogue/SingleNormalOrder/EditinginReview',function()
{
  var orderid = '';
  var invoiceid= '';
  var labelpdf = '';
...
cy.get('.rs-input').click().type('name012222')
...
cy.wait(1000)
cy.get('.rs-flex-box-grid-space-between > :nth-child(1) > .Flexbox_flex-row__aKbHb').click()
cy.wait(4000)
cy.get(':nth-child(5) > :nth-child(1) > .Input_input-group__c6y0f > .rs-input').clear().type('7878787879')
cy.get(':nth-child(6) > :nth-child(1) > .Input_input-group__c6y0f > .rs-input').clear().type('Pratik')
cy.get('.rs-btn-toolbar > :nth-child(2)').click()
cy.get('.Flexbox_mt-lg__HQln6 > .Button_button-primary__9i0Rz').click()
cy.wait(4000)
cy.get('.rs-col-xl-18 > .Card_card__DkrpZ > .Flexbox_flex-row__aKbHb > .rs-btn-toolbar > .Button_button-primary__9i0Rz').click()
...
{
    cy.get('.rs-drawer-content').eq(index).click().type('test123')
}
})
