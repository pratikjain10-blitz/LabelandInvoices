...
describe("MultipleRegeneration", function () {
  ...
  it("Expired Single AWB", function () {
    ...
    cy.get(".ExpiredAWBOrdersCard_card__2yYch > :nth-child(2)").each(
      (
        $p1,
        number,
        $queue //getting the expired cards
      ) => {
        if (number <= 5) {
          cy.get(
            ".ExpiredAWBOrdersCard_card__2yYch > :nth-child(1) > .ExpiredAWBOrdersCard_card-cell-checkbox__NX5dd > .Flexbox_flex-column__cNkZ2 > .rs-checkbox > .rs-checkbox-checker > label > .rs-checkbox-wrapper"
          )
            .eq(number)
            .click();
        }
  ...
  it("Expired Clubbed AWB", function () {
    ...
    cy.get(".rs-btn-toolbar > .Button_button-ghost__rieSu")
      ...
      .click();
    cy.get(
      'div[class="Flexbox_flex-row__aKbHb Flexbox_align-middle__-J0b5 ExpiredAWBOrdersCard_clubbed-parent-card__1-FJb rs-flex-box-grid rs-flex-box-grid-top rs-flex-box-grid-start"]'
    ).each(($p1, number, $queue) => {
      if (number <= 5) {
        cy.get(
          " .ExpiredAWBOrdersCard_clubbed-parent-card__1-FJb > .rs-checkbox > .rs-checkbox-checker > label > .rs-checkbox-wrapper"
        )
          .eq(number)
          .click();
      }
