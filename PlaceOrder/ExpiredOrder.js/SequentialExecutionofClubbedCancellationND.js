...
//This requires editing to Cancellation flow currently loop holes is
//remove regenerate and put cancel flow in place
//add going to single when less than 2 orders
//add going to clubbed when more than 2 orders are there
//which order to be cancelled and which to be captured is to sorted
            ...
            if (count === 2) {
              cy.get('div[class="ExpiredAWBOrdersCard_card-border__7Ruq8"]')
                ...
                .within(() => {
                  ...
                  cy.get(
                    ":nth-child(1) > .rs-col-xxl-3 > .action-btns-wrapper > .button-link"
                  ).then(($v1) => {
                    cy.get(
                      ":nth-child(1) > .rs-col-xxl-3 > .action-btns-wrapper > .button-link"
                    )
              ...
              cy.get("@order2")
                      ...
                      cy.get(".rs-drawer-body").within(() => {
                        cy.get(
                          ...
                          if (p === order2) {
                            ...
                            cy.wait(1000);
                            cy.get(
                              ".rs-picker-toggle-placeholder > .Text_body2__0FftJ"
                            ).click({ force: true });
                ...
                .then(() => {
                  cy.get('span[class="rs-picker-select-menu-item"]')
                        ...
                        .each(($p1, number, $queue) => {
                          cy.get(
                            ...
                            .within(() => {
                              ...
                              //capturing the clubbed order id here as also after regeneration its a clubbed order
                              cy.get(
                          ...
                          cy.get("@order2").then((order2) => {
                            cy.get("@AWB").then((AWB) => {
                              cy.get("@orderid").then((orderid) => {
                                cy.get("@label").then((label) => {
                                  cy.log(AWB);
                                  const awb = AWB.substring(4, AWB.length);
                                  cy.log(awb);
                                  cy.log(label);
                                  const labellink = label.substring(
                                    51,
