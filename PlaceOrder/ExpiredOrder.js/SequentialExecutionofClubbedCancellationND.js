...
{ 
    ...
    { 
      ...
      cy.get('.rs-radio-wrapper').eq(index).click()
          ...
          cy.get('div[class="ExpiredAWBOrdersCard_card-border__7Ruq8"]').each(($e1, index, list) => {
            let count;
          cy.get('div[class="ExpiredAWBOrdersCard_card-border__7Ruq8"]').eq(index).within(() => {
          ...
          //cancel loop starts here
            ...
            cy.get(':nth-child(1) > .rs-col-xxl-3 > .action-btns-wrapper > .button-link').then(($v1)=>{
            cy.get(':nth-child(1) > .rs-col-xxl-3 > .action-btns-wrapper > .button-link').eq(0).click()
            cy.wait(2000)
            })
          })
            ...
            cy.log(p)
             if(p===order2)
             {
              ...
              cy.wait(1000)
              cy.get('.rs-picker-toggle-placeholder > .Text_body2__0FftJ').click({ force: true })                                                                                   
 ...
 {  
  ...
  cy.get('@label').then((label)=>
  {
    ...
    cy.wait(10000)
    cy.task('readPdf',PdfContent).should('contain', order2)
