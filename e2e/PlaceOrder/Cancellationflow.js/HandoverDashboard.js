...
describe("NONduplicateOrderflow", function () {
  ...
  it("Expired Clubbed AWB", function () {
    ...
    cy.get(".rs-btn-toolbar > .Button_button-ghost__rieSu")
      ...
      .click()
      .then(() => {
        cy.get('div[class="HandoverChildViewCard_card__6hKXd"]').each(
          ($e1, index, list) => {
            cy.get(
              ".HandoverCard_flex-box-wrapper__PRITZ > .HandoverCard_right-arrow-icon__kBHoD"
            )
              .eq(index)
              .click();
            cy.wait(2000);
            cy.get('div[class="HandoverChildViewCard_card__6hKXd"]').each(
              ($p1, num, sequence) => {
                cy.get(
                  '.Flexbox_flex-row__aKbHb > .rs-flex-box-grid-item-5 > [data-sd-event="detailsHandover"]'
                )
                  .eq(num)
                  .click()
                  .then(() => {
                    cy.get(
                      'div[class="HandoverChildViewCard_card__6hKXd"]'
                    ).then(() => {
                      cy.get(
                        ":nth-child(4) > :nth-child(1) > .Text_headings-colored__kF2dK"
                      ).then(($a1) => {
                        cy.wrap($e1).as("AWB");
                      });
                      cy.get(
                        ".rs-drawer-body > :nth-child(2) > :nth-child(1) > .Text_headings-colored__kF2dK"
                      ).then(($b1) => {
                        cy.wrap($b1).as("Clubbedorderid");
                      });
                      // cy.get('.KeyValue_wrapper__vJVde > :nth-child(6)').each(($c1,index,list)=>{
                      cy.get(".KeyValue_wrapper__vJVde > :nth-child(6)")
                        .its("length")
                        .then(($z) => {
                          count = $z;
                          count = count - 1;
                          if (count === 2) {
                            cy.get(
                              ".KeyValue_wrapper__vJVde > :nth-child(6)"
                            ).each(($c1, index, list) => {
                              let order = $c1.text();
                            ...
                            cy.get(
                              ...
                              .then(() => {
                                cy.get(
                                  ".Flexbox_flex-row__aKbHb > .rs-flex-box-grid-item-5 > .button-link"
                                )
                                  .contains("Cancel")
                                  .click();
                              });

                            cy.get(".rs-modal-content")
                              .within(() => {
                                cy.get(".rs-modal-footer > :nth-child(2)")
                              ...
                              .then(() => {
                                cy.get(".rs-drawer-body").within(() => {
                                  cy.get(
                                    ":nth-child(1) > .rs-flex-box-grid-item-8 > :nth-child(2)"
                                  ).each(($v1, index, list) => {
                                    let p = $v1.text();
                                    cy.log(p);
                                    if (p === id2) {
                                      cy.get(
                                        ":nth-child(1) > .rs-flex-box-grid-item-3 > .rs-checkbox > .rs-checkbox-checker > label > .rs-checkbox-wrapper"
                                      )
                                        .eq(index)
                                        .click();
                                      cy.wait(1000);
                              ...
                              .then(() => {
                                cy.get(
                                  'span[class="rs-picker-select-menu-item"]'
                                )
                                  .contains("Doubtful customer")
                                  .click();
                                cy.get(
                                ...
                                cy.get(
                                  'div[class="PrintPackViewCard_card-border__Qk1I3"]'
                                )
                                  .eq(0)
                                  .then(() => {
                                    cy.get(
                                    ...
                                    cy.get(
                                      ".rs-col-lg-4.rs-col-sm-7 > :nth-child(5)"
                                    )
                                      .eq(0)
                                      .then(($e1) => {
                                        let orderid = $e1.text();
                                        ...
                                        cy.get("@NewAWB").then((NewAWB) => {
                                          cy.get("@orderid").then((orderid) => {
                                            cy.get("@label").then((label) => {
                                              ...
                                              cy.log(awb);
                                              cy.log(label);
                                              const labellink = label.substring(
                                                51,
                                                label.length
                                              );
                                              const downloadPath =
                            ...
                            cy.get(".rs-modal-content")
                              ...
                              .then(() => {
                                ...
                                cy.get(
                                  ...
                                  .click();
                                cy.reload();
                                cy.get(".Button_button-default__NeJ4p").each(
                                  ($x1, index, list) => {
                                    if ($x1.text().includes("Clubbed")) {
                                      cy.get(".Button_button-default__NeJ4p")
                                        .eq(index)
                                        .click();
                                    }
                                  }
